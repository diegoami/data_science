...
def fix_turnstile_data(filenames):
    ...
    for name in filenames:
        f_in = open(name,'r',newline='')
        f_out = open('updated_'+name,'w',newline='')
        ...
        for line in reader_in:
            for i in range(0,8):
                if (len(line) > 7+5*i):
                    writer_out.writerow([line[0], line[1], line[2], line[3+5*i], line[4+5*i], line[5+5*i], line[6+5*i], line[7+5*i]])
        f_in.close()
        f_out.close()
